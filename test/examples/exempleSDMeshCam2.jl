...
global const window = createwindow("Mesh Display", 1000, 1000, debugging = true)
                                                   # debugging just works on linux and windows
...
obj = RenderObject( @compat( Dict{Symbol,Any}(
    :vertex           => verts,
...
lines = RenderObject( @compat( Dict{Symbol,Any}(
    :vertex           => verts,
...
println("Before Loop")        
#@async
       while true # !GLFW.WindowShouldClose(window.glfwWindow)
	  println("In Loop")        
